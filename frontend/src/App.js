...
function App() {
  ...
  // 💡 NUEVO: FUNCIÓN PARA BORRAR PARTIDO
  const handleDeleteMatch = async (matchId) => {
    if (window.confirm("⚠️ ¿Estás seguro de que quieres eliminar este partido? Esta acción borrará el archivo de tu ordenador y no se puede deshacer.")) {
      try {
        ...
        fetchMatchesList(); // Refresca la lista tras borrar
      } catch (err) {
            ...
            {savedMatches.length === 0 ? (
              ...
              savedMatches.map(m => (
                <div key={m.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", backgroundColor: "#34495e", padding: "20px", borderRadius: "10px", border: "1px solid #7f8c8d" }}>
                  ...
                  {/* 💡 NUEVO: BOTONES DE CARGAR Y BORRAR */}
  ...
  return (
    <div style={{ display: "flex", gap: "20px", padding: "20px", backgroundColor: "#2c3e50", minHeight: "100vh", fontFamily: "Arial, sans-serif", position: "relative" }}>
      ...
      <div style={{ flex: "0 0 1050px", display: "flex", flexDirection: "column", paddingTop: "30px" }}>
        ...
        <div style={{ display: "flex", gap: "10px", marginBottom: "15px", backgroundColor: "#34495e", padding: "10px", borderRadius: "8px" }}>
          <button onClick={() => changePeriod("total")} style={{ flex: 1, padding: "12px", borderRadius: "5px", border: "none", fontWeight: "bold", fontSize: "16px", cursor: "pointer", transition: "0.2s", backgroundColor: selectedPeriod === "total" ? "#27ae60" : "#2c3e50", color: "white" }}>
            Partido Completo
          </button>
          <button disabled={!hasH2} onClick={() => changePeriod("h1")} style={{ flex: 1, padding: "12px", borderRadius: "5px", border: "none", fontWeight: "bold", fontSize: "16px", cursor: hasH2 ? "pointer" : "not-allowed", transition: "0.2s", backgroundColor: selectedPeriod === "h1" ? "#27ae60" : "#2c3e50", color: hasH2 ? "white" : "#7f8c8d" }}>
            1ª Parte
          </button>
          <button disabled={!hasH2} onClick={() => changePeriod("h2")} style={{ flex: 1, padding: "12px", borderRadius: "5px", border: "none", fontWeight: "bold", fontSize: "16px", cursor: hasH2 ? "pointer" : "not-allowed", transition: "0.2s", backgroundColor: selectedPeriod === "h2" ? "#27ae60" : "#2c3e50", color: hasH2 ? "white" : "#7f8c8d" }}>
            2ª Parte
          </button>
        </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", borderTop: "1px solid #ccc", paddingTop: "15px", paddingBottom: "15px" }}>
            ...
            <div style={{ display: "flex", gap: "15px", justifyContent: "space-between" }}>
              ...
              <div style={{ flex: 1, backgroundColor: "#fde8c4", padding: "10px", borderRadius: "8px", border: "1px solid #e2bc82", textAlign: "center" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-around" }}>
                  <div><div style={{ fontSize: "12px", color: "#666" }}>Dist. ({selectedPeriod.toUpperCase()})</div><div style={{ fontSize: "18px", fontWeight: "bold" }}>{stats.distance.toFixed(0)}m</div></div>
